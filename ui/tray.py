...
from core.updater import check_for_update, download_url_for
...
class App:
    def __init__(self):
        ...
        self.app_state["history"] = history_module
    ...
    def perform_optimization(self, manual=False):
        ...
        if manual and self.config.get("auto_restore_point", False):
            try:
    ...
    def call_in_tk(self, func):
        if self.tk_root is None:
            return
        self._tk_queue.put(func)
        try:
            self.tk_root.event_generate("<<TrayEvent>>", when="tail")
        except Exception:
            pass
    ...
    def _apply_windows_theme(self):
        try:
            ...
            winreg.CloseKey(key)
            desired = "light" if value == 1 else "dark"
            if desired != self.theme_manager.current_theme_name:
                self.theme_manager.set_theme(desired)
                save_config(self.config)
                if self._main_window_open:
                    self._close_main_window()
                    self.call_in_tk(self._do_open_window)
        except Exception:
            pass
    ...
    def _tk_pump(self):
        ...
        if self.tk_root is not None:
            self.tk_root.after(200, self._tk_pump)
    ...
    def on_autostart(self, icon, item):
        ...
        if success:
            ...
            icon.notify(msg, t(lang, "app_name"))
            logger.info(f"Автозагрузка: {'вкл' if new_state else 'выкл'}")
        else:
            logger.error("Не удалось изменить автозагрузку")

    def on_stats(self, icon, item):
        if not self.tk_root:
            return
        self.call_in_tk(self._do_open_stats)
    ...
    def _close_main_window(self):
        toplevel = getattr(self.main_window, '_toplevel', None)
    ...
    def on_check_update(self, icon, item):
        icon.notify(t(self._get_lang(), "checking_update"), t(self._get_lang(), "app_name"))
    ...
    def _check_update_thread(self):
        try:
            ...
            if release:
                url = download_url_for(release)
                import webbrowser
                webbrowser.open(url)
                self.icon.notify(
                    f"New version available: {release.get('tag', '')}",
                    t(self._get_lang(), "app_name")
                )
            else:
                self.icon.notify(
                    t(self._get_lang(), "up_to_date"),
                    t(self._get_lang(), "app_name")
                )
        except Exception as e:
    ...
    def on_exit(self, icon, item):
        ...
        if self.tk_root:
            try:
                self.call_in_tk(self.tk_root.destroy)
            except Exception:
                pass
    ...
    def _build_menu(self):
        ...
        submenu_help = pystray.Menu(
            ...
            pystray.MenuItem(t(lang, "check_update"), self.on_check_update),
    ...
    def run_with_tk(self):
        ...
        self.tk_root.bind("<<TrayEvent>>", lambda e: self._tk_pump())
