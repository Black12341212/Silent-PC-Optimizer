...
class SystemMonitor:
    ...
    def _get_cpu_temperature(self):
        try:
            wmi_obj = __import__("wmi", fromlist=["WMI"])
            w = wmi_obj.WMI(namespace="root\\OpenHardwareMonitor")
            sensors = w.Sensor()
        ...
        try:
            wmi_obj = __import__("wmi", fromlist=["WMI"])
            w = wmi_obj.WMI(namespace="root\\WMI")
...
def get_network_processes(top_n=10):
    net_procs = []
    try:
        ...
        pass
    net_procs.sort(key=lambda p: p["total_mb"], reverse=True)
    return net_procs[:top_n]
