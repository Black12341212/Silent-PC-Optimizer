...
def is_ssd(drive_letter="C"):
    try:
        ...
        ps = (
            f"$vol = Get-Volume -DriveLetter {letter} -ErrorAction SilentlyContinue; "
            "if ($vol) { "
            "$disk = $vol | Get-Partition | Get-Disk | Select-Object -First 1; "
        ...
        if "ssd" in output or "non-rotating" in output:
            return True
        if "hdd" in output or "rotating" in output or "unspecified" in output:
            return False
